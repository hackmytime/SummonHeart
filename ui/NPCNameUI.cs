        ...
        public override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < NPCName.Count; i++)
            {
                if (NPCName[i].Color != Color.Black)
                    npanel = new UIText(NPCName[i].Text, 0.9f / Mathf.Pow(SummonHeartMod.zoomValue.Y, 0.5f))
                    {
                        TextColor = NPCName[i].Color
                    };
                else
                {
                    npanel = new UIText(NPCName[i].Text, 0.75f / Mathf.Pow(SummonHeartMod.zoomValue.Y, 0.5f))
                    {
            ...
            for (int k = 0; k < 200; k++)
            {
                ...
                if (ActualNPC.active && !ActualNPC.dontCountMe)
                {
                    ...
                    if (IsMouseOver)
                    {
                        ...
                        RPGGlobalNPC Rnpc = ActualNPC.GetGlobalNPC<RPGGlobalNPC>();
                        string preFix = "";
                    ...
                    else if (IsOnScreen && (Config.vConfig.DisplayNpcName && !ActualNPC.townNPC || Config.vConfig.DisplayTownName && ActualNPC.townNPC))
                    {
                        ...
                        if (Rnpc.getLevel >= 0 && ActualNPC.damage > 0)
                        {
                            preFix = Rnpc.GetLevelText();
                            preFix = "x" + Rnpc.life;
                            /*
                            if (NPCUtils.GetWorldTier(ActualNPC, Rnpc.getLevel) > 0)
                                preFix += "( + " + NPCUtils.GetWorldTier(ActualNPC, Rnpc.getLevel) + " )";
                            */
                        }
                        ...
                        int NpcColor = 0;
                        int LevelOffSet = ActualNPC.GetGlobalNPC<RPGGlobalNPC>().getLevel + ActualNPC.GetGlobalNPC<RPGGlobalNPC>().getTier - Main.LocalPlayer.GetModPlayer<RPGPlayer>().GetLevel();
                        if (LevelOffSet < -100)
                            NpcColor = 0;
                        ...
                        switch (ActualNPC.GetGlobalNPC<RPGGlobalNPC>().getRank)
                        {
                            case 0:
                                NpcColor--;
                                break;
                        ...
                        if(ActualNPC.life < ActualNPC.lifeMax)
